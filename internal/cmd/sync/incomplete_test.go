...
func TestAFailedPushRendersAnIncompleteReport(t *testing.T) {
	...
	out, err := captureStdout(t, func() error {
		return cmdSync(context.Background(), fakeClient(f), "push", []string{"proj-A", dir})
	})
	if err == nil {
...
func TestASuccessfulPullJSONHasNoIncompleteKey(t *testing.T) {
	...
	out, err := captureStdout(t, func() error {
		return cmdSync(context.Background(), fakeClient(f), "pull", []string{"proj-A", dir, "--json"})
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestStatusJSONOnAnErrorPathIsNotALoneReport(t *testing.T) {
	...
	out, err := captureStdout(t, func() error {
		return cmdSync(context.Background(), fakeClient(f), "status", []string{"proj-A", dir, "--json"})
	})
	if err == nil {
		t.Fatal("want an error")
	}
