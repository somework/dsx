...
- **`cmdSync` builds its FlagSet from a variable, so `flagSetOwners` attributes `--prune/--force/-n/-q/-j` to every command in `synccmd.Group`.** Adding any command there fails `TestEveryDeclaredFlagIsDocumented` until `cmdSync` is split into three literal `NewFlagSet("pull"/"push"/"status")` calls. Widening the `usageFooter` scope instead turns the test green and `dsx help` wrong — the backward half reads the same rewritten set, so the guard would end up certifying a lie.
