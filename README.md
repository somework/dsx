...
The offline suite needs no login and reaches no network — it is the one to run. The last two
lines are their own CI jobs, so the first two can be green while either is not.
